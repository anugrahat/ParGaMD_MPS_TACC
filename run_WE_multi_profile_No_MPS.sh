#!/bin/bash
#SBATCH --job-name=gamd_run_no_mps
...
# Here, we call "nsys profile" and pass an inline script via bash -c "..."
# This entire block is what was previously in your run_WE_multi_xxx.sh.
# We DISABLE MPS by commenting out those lines, so this is your "no MPS" run.

# We use --force-overwrite true so we don't get "file already exists" errors.
...
##############################################################################
# 1) MPS is DISABLED for No MPS run
##############################################################################
...
##############################################################################
# 7) Clean up MPS -- DISABLED for No MPS run
##############################################################################
# echo \"Stopping MPS...\"
